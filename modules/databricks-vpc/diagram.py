...
with Diagram("Tecton Databricks VPC Deployment", graph_attr=diagram_attr, filename="diagram"):
    tecton = Edge(Custom("Tecton", "../../resources/tecton-red-on-blue.png"))
    databricks = Edge(Databricks("Databricks"))

    with Cluster("Customer owned AWS account"):
        S3("Tecton S3 bucket")
        with Cluster("IAM"):
            ca_iam_role = IAM("Cross-account role")
            databricks_ca_iam_role = IAM("Databricks cross-account role")
    ...
    tecton >> ca_iam_role
    databricks >> databricks_ca_iam_role
