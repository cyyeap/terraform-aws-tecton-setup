...
variable "tecton_vpc_cidr_block" {
  type        = string
  default     = "10.64.0.0/16"
  description = <<EOV
  The VPC cidr block for the private and public subnets which will be created. Both
`tecton_vpc_cidr_block` and `emr_vpc_cidr_block` will be associated with the created (or specified)
VPC. The smallest acceptable prefix is /18.
EOV

  validation {
    condition     = tonumber(regex("/([0-9]+)", var.tecton_vpc_cidr_block)[0]) <= 18
    error_message = "Subnet must have enough space: the smallest acceptable prefix is /18."
  }
}

variable "emr_vpc_cidr_block" {
  type        = string
  default     = "10.38.0.0/16"
  description = <<EOV
  The VPC cidr block for the private and public subnets which will be created. Both
`emr_vpc_cidr_block` and `tecton_vpc_cidr_block` will be associated with the created (or specified)
VPC. The smallest acceptable prefix is /18.
EOV

  validation {
    condition     = tonumber(regex("/([0-9]+)", var.emr_vpc_cidr_block)[0]) <= 18
    error_message = "Subnet must have enough space: the smallest acceptable prefix is /18."
  }
}
...
variable "enable_emr_vpc_cidr_block_association" {
  type        = bool
  default     = true
  description = <<EOV
...
variable "enable_tecton_vpc_cidr_block_association" {
  type        = bool
  default     = true
  description = <<EOV
Only used when `vpc_id` is specified. Toggle enabling the
