...
output "private_subnet_ids_by_az" {
  description = "A map of private subnet ids by their respective availability zone. (`{availability_zone_name: subnet_id}`)"
  value = zipmap(
    slice(data.aws_availability_zones.available.names, 0, local.availability_zone_count),
...
output "public_subnet_ids_by_az" {
  description = "A map of public subnet ids by their respective availability zone. (`{availability_zone_name: subnet_id}`)"
  value = zipmap(
    slice(data.aws_availability_zones.available.names, 0, local.availability_zone_count),
